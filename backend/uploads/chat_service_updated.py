...
async def process_chat_message(
    # Check if the last message role is user, ensure alternation
    last_message = await get_last_message(chat_id)
    if last_message and last_message['role'] == 'user':
        raise ValueError('Invalid role sequence: multiple user messages in a row are not allowed.')
    ...
    Yields:
        str: Respons yang dihasilkan untuk pesan pengguna.
    """
    ...
    try:
        ...
        if chat_id is None:
            ...
            chat_id = chat.create_chat(user_id)
        ...
        # Tambahkan pesan user ke database
        if file_id:
            chat.add_message(chat_id, message, is_user=True, file_id=file_id)
        else:
